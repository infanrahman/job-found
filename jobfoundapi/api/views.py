...
def Viewemployeesadmin(request,id):
    p=Employee.objects.filter(category=id)
    if request.method=='POST':
        nm=request.POST['date']
        print(nm)
        cat=Employee.objects.filter(date=nm,category=id)
        print(cat)
        return render(request,"admin/Viewemployeesadmin.html", {'cat':cat,'n':nm})
    return render(request,"admin/Viewemployeesadmin.html",{'p': p})
...
def Placeview(request,id):
    if request.method == "POST":
        ...
        employee=Employee.objects.filter(category=id,pincode=pincode)
        serializer=EmployeeSerializers(employee,many=True)
        print(serializer)
        return JsonResponse({"Employees": serializer.data}, safe=False, status=status.HTTP_200_OK)
...
def getfile(request):  
    ...
    for employee in employees:  
        writer.writerow([employee.user_id,employee.lastname,employee.category,employee.description,employee.address,
               employee.phone,employee.state,employee,employee.district,employee.pincode,employee.Adhaar_no,
               employee.adhaar_img,employee.password,employee.employee_status,
               employee.longitude,employee.latiude,employee.place])  
    return response  
